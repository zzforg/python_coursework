...
class PlotApp(object):
    ...
    def drawlines(self):
        ...
        if self.getPoints is None: return 
        self.worldscreen = WorldScreen(self.getPoints[0], self.getPoints[2], self.getPoints[1], self.getPoints[3], self.canvasWidth, self.canvasHeight)
        if self.worldscreen is None: return
        ...
        for func in  self.storeFuncColor:
            ...
            for worldX, worldY in FunctionIterator(func[0], self.getPoints[0], self.getPoints[1], self.getPoints[4]):
                screenCoord = self.worldscreen.world2screen(worldX,worldY)
    ...
    def mouseCursor(self, event):
        ...
        mouseCoord = self.worldscreen.screen2world(event.x, event.y)
